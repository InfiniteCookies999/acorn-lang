...
acorn::ArrayType* acorn::ArrayType::create(PageAllocator& allocator,
    ...
    new (arr_type) ArrayType(is_const, expr, elm_type);
    ...
    arr_type->determined_length = false;
    return arr_type;
}
...
acorn::ArrayType* acorn::ArrayType::create(PageAllocator& allocator,
                                           ...
                                           bool is_const) {
    ArrayType* arr_type = allocator.alloc_type<ArrayType>();
    new (arr_type) ArrayType(is_const, length, elm_type);
    arr_type->contains_const = is_const;
    arr_type->contains_generics = elm_type->does_contain_generics();
    if (!is_const) {
        arr_type->non_const_version = arr_type;
    }
    arr_type->determined_length = true;
    return arr_type;
}
...
acorn::Type* acorn::AssignDeterminedArrayType::create(PageAllocator& allocator,
    ...
    arr_type->contains_const = is_const;
    arr_type->contains_const = elm_type->does_contain_generics();
    return arr_type;
}
...
acorn::Type* acorn::UnresolvedCompositeType::create(PageAllocator& allocator,
    ...
    new (composite_type) UnresolvedCompositeType(is_const, name, name_location, std::move(generic_exprs));
    composite_type->contains_const = is_const;
    if (!is_const) {
        composite_type->non_const_version = composite_type;
    }
